					 ...
					 struct list_head *pagepool)
{
	...
	for (i = j = 0; i < nr; ++i, ++j) {
		...
		if (test_bit(j, bounced)) {
			BUG_ON(i < lz4_max_distance_pages);   /* DBG_BUGON -> BUG_ON */
			BUG_ON(top >= lz4_max_distance_pages);   /* DBG_BUGON -> BUG_ON */
...
static int z_erofs_lz4_decompress(struct z_erofs_decompress_req *rq, u8 *dst)
{
	...
	BUG_ON(*rq->in == NULL);   /* DBG_BUGON -> BUG_ON */
				      ...
				      struct list_head *pagepool)
{
	...
	if (rq->inputsize <= PAGE_SIZE) {
		if (nrpages_out == 1 && !rq->inplace_io) {
			BUG_ON(!*rq->out);   /* DBG_BUGON -> BUG_ON */
				     ...
				     struct list_head *pagepool)
{
	...
	if (rq->out[0] == *rq->in) {
		BUG_ON(nrpages_out != 1);   /* DBG_BUGON -> BUG_ON */
		return 0;
	}
	...
	if (nrpages_out == 2) {
		BUG_ON(!rq->out[1]);   /* DBG_BUGON -> BUG_ON */
		if (rq->out[1] == *rq->in) {
