...
// #include <trace/events/erofs.h>   /* غير متوفر في 3.18 */
...
static int z_erofs_create_pcluster_pool(void)
{
	...
	for (pcs = pcluster_pool;
		...
		/* استبدال struct_size بحساب يدوي */
		size = offsetof(struct z_erofs_pcluster, compressed_pages) +
		       pcs->maxpages * sizeof(struct page *);
...
/* تم تبسيط: لا نستخدم pcpu kthread ولا hotplug */
				     ...
				     struct list_head *pagepool)
{
	...
	for (; index < pcl->obj.index + pcl->pclusterpages; ++index, ++pages) {
		...
		if (!cmpxchg(pages, NULL, tagptr_cast_ptr(t)))
			continue;
				     ...
				     struct erofs_map_blocks *map)
{
	...
	if (length & Z_EROFS_PCLUSTER_FULL_LENGTH) {
		...
		while (llen > length &&
		       length != cmpxchg(&pcl->length, length, llen)) {
			cpu_relax();
			length = READ_ONCE(pcl->length);
		}
	}
				   ...
				   struct erofs_map_blocks *map)
{
	...
	BUG_ON(clt->owned_head == Z_EROFS_PCLUSTER_NIL);
	BUG_ON(clt->owned_head == Z_EROFS_PCLUSTER_TAIL_CLOSED);
				       ...
				       bool sync, int bios)
{
	...
	if (atomic_add_return(bios, &io->pending_bios))
		return;
	/* Use workqueue always (no pcpu kthread) */
...
static void z_erofs_decompressqueue_endio(struct bio *bio)
{
	...
	bio_for_each_segment_all(bvec, bio, i) {
		...
		BUG_ON(z_erofs_page_is_invalidated(page));
				     ...
				     struct list_head *pagepool)
{
	...
	while (owned != Z_EROFS_PCLUSTER_TAIL_CLOSED) {
		...
		BUG_ON(owned == Z_EROFS_PCLUSTER_TAIL);

		/* no possible that 'owned' equals NULL */
		BUG_ON(owned == Z_EROFS_PCLUSTER_NIL);
...
static void z_erofs_decompressqueue_work(struct work_struct *work)
{
	...
	BUG_ON(bgq->head == Z_EROFS_PCLUSTER_TAIL_CLOSED);
...
repeat:
	...
	BUG_ON(justfound && PagePrivate(page));
				    ...
				    z_erofs_next_pcluster_t owned_head)
{
	...
	BUG_ON(owned_head == Z_EROFS_PCLUSTER_TAIL_CLOSED);
				 ...
				 bool *force_fg)
{
	...
	do {
		...
		BUG_ON(owned_head == Z_EROFS_PCLUSTER_TAIL_CLOSED);
		BUG_ON(owned_head == Z_EROFS_PCLUSTER_NIL);
...
submit_bio_retry:
			...
			if (!bio) {
				/* في 3.18: bio_alloc(gfp, nr_iovecs) */
				...
				/* في 3.18 نستخدم bi_rw بدلاً من bi_opf */
				bio->bi_rw = READ;
				++nr_bios;
			}
...
static int z_erofs_readpage(struct file *file, struct page *page)
{
	...
	/* trace_erofs_readpage(page, false); تم إزالة التتبع */
